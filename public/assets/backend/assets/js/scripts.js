(function (window, undefined) {
  ...
  // stripe payment
  var stripe = Stripe('pk_test_51NH6R0E6WLqdeM6ySn4ZsQkuyuTereKGIOt3vj5mu7wOhSlSAZyiHWDhPrjqDIn6m0YyVFtmjFEnF3Dp2zxw7B9M00F0U6byDQ');
  ...
  form.addEventListener('submit', function(event) {
    ...
    stripe.createToken(card).then(function(result) {
      if (result.error) {
        ...
        $.ajax({
          type: 'get',
          url: $("#subscription #payment_url").val() + "/api/apps/process_payment",
          headers: {
            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            },
          data: {stripe_token: result.token, plan_id: $("#subscription #plan_id").val(), app_id: $("#subscription #app_id").val(), payMonths: $("#subscription #pay_amounts").val()},
          success: (res) => {
            if (res.success) {
              Swal.fire({
                icon: 'success',
  ...
  $("#subscription #pay_amounts").change(function () {
    $("#subscription #total_price").text($(this).val() * $("#subscription #price").val());
  });
